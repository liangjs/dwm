...
enum { SchemeNorm, SchemeSel, SchemeStatus, SchemeTagsSel, SchemeTagsNorm, SchemeInfoSel, SchemeInfoNorm, SchemeSystray }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - sw - stw - x - title_gap) > bh) {
			...
			if ((code = XKeysymToKeycode(dpy, keys[i].keysym))) {
				if (keys[i].chain != -1 &&
					((chain = XKeysymToKeycode(dpy, keys[i].chain))))
						code = chain;
				for (j = 0; j < LENGTH(modifiers); j++)
					XGrabKey(dpy, code, keys[i].cmod | keys[i].mod | modifiers[j], root,
...
keypress(XEvent *e)
{
	...
	for (i = 0; i < LENGTH(keys); i++) {
		...
		else if (keysym == keys[i].chain
				&& CLEANMASK(keys[i].cmod) == CLEANMASK(ev->state)
				&& keys[i].func) {
			current = 1;
			keychain = keysym;
			for (j = 0; j < LENGTH(modifiers); j++)
				XGrabKey(dpy, AnyKey, keys[i].mod | modifiers[j], root,
						True, GrabModeAsync, GrabModeAsync);
...
togglefloating(const Arg *arg)
{
	...
	if (selmon->sel->isfloating)
		...
		/* save last know float dimensions */
